...
compute first, then scan, then absence, then timeline, else hybrid.
`expected_mode` is the first classifier that fires in that order.
...
CASES = [
    ...
    # graph-relationship phrasing must NOT steal timeline (stays hybrid; graph is
    # explicit-only) — the roadmap's timeline<->graph boundary
    ("what depends on auth.py", "hybrid"),
