...
@records_group.command("recompute-salience")
@click.option(
    "--url",
    envvar="BRAINPALACE_URL",
    default=None,
...
@click.option(
    "--metric", "-m", default=None, help="Restrict recompute to this metric only."
)
@click.option("--json", "json_output", is_flag=True, help="Output as JSON")
def records_recompute_salience(
    url: str | None, metric: str | None, json_output: bool
) -> None:
    """Re-score record salience with the registered scorer."""
    resolved_url = url or get_server_url()
    body: dict[str, str] = {}
    ...
    try:
        with DocServeClient(base_url=resolved_url) as client:
            r = client._request("POST", "/records/recompute-salience", json=body)
            if json_output:
                import json as _json
                ...
                console.print(
                    f"[green]Recomputed salience for {r['rescored']} record(s).[/]"
                )
    except ConnectionError as e:
        exit_on_connection_error(e, base_url=resolved_url, json_output=json_output)
    except ServerError as e:
