...
def _source_dir(dist_name: str = "brainpalace-cli") -> str | None:
...
def _source_git_ref(dist_name: str = "brainpalace-cli") -> str | None:
...
def version_display() -> str:
    """Version string for ``--version``. For a local source build, appends the
    build's git ref — ``(from <branch> <short-commit>)`` — so it is distinguishable
    from a same-numbered PyPI release (the number alone cannot tell them apart) and
    pinpoints the exact built commit. Falls back to ``(from source)`` when git info
    is unavailable. ``__version__`` itself stays pure for version comparisons.
    """
