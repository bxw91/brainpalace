...
def test_prompts_shown_and_decline_is_config_only(tmp_path, monkeypatch):
    ...
    # reranker-change=N, lemma=N, estimate?=N, proceed=N
...
def test_estimate_gate_precedes_final_proceed_on_fresh_start(tmp_path, monkeypatch):
    ...
    # graphrag-extract=N, reranker-change=N, lemma=N, estimate?=N, proceed=Y
...
def test_explicit_flags_skip_prompts(tmp_path, monkeypatch):
    ...
    # gate (N = keep inherited), lemma (N), estimate gate (N = skip), then the
    # final proceed confirm (N).
    r = _invoke(
        tmp_path,
        monkeypatch,
        args=[
