"""Tests for the Antigravity (agy) runtime converter.

Mirrors test_codex_converter.py — AntigravityConverter is architecturally a
direct mirror of CodexConverter (skill-runtime flatten + AGENTS.md), so its
test coverage should match Codex's, adjusted for the Antigravity target
directory (.agents/skills/brainpalace) and header text.
"""
...
from brainpalace_cli.runtime.antigravity_converter import (
    AGENTS_MD_END,
    AGENTS_MD_START,
    AntigravityConverter,
    _add_antigravity_header,
    _update_agents_md,
)
...
class TestAntigravityConverter:
    """Tests for AntigravityConverter."""

    def test_runtime_type(self) -> None:
        converter = AntigravityConverter()
        assert converter.runtime_type == RuntimeType.ANTIGRAVITY

    def test_convert_command_has_antigravity_header(
        self, sample_command: PluginCommand
    ) -> None:
        converter = AntigravityConverter()
        result = converter.convert_command(sample_command)
        assert "Antigravity Skill:" in result
        assert "brainpalace-search" in result

    def test_convert_agent_has_antigravity_header(
    ...
    def test_install_creates_antigravity_structure(
        self, tmp_path: Path, sample_bundle: PluginBundle
    ) -> None:
        """Test Antigravity install creates skills + AGENTS.md."""
    ...
    def test_install_antigravity_headers_in_skills(
        self, tmp_path: Path, sample_bundle: PluginBundle
    ) -> None:
        """Test that command/agent SKILL.md files have Antigravity headers."""
        target = tmp_path / ".agents" / "skills" / "brainpalace"
        converter = AntigravityConverter()
        converter.install(sample_bundle, target, Scope.PROJECT)

        cmd_skill = (target / "brainpalace-search" / "SKILL.md").read_text()
        assert "Antigravity Skill:" in cmd_skill

        agent_skill = (target / "brainpalace-search-assistant" / "SKILL.md").read_text()
        assert "Antigravity Skill:" in agent_skill
...
class TestAddAntigravityHeader:
    """Tests for Antigravity header injection."""

    def test_adds_header_after_frontmatter(self) -> None:
        content = "---\nname: test\n---\nBody content.\n"
        result = _add_antigravity_header(content, "test-cmd")
        assert "Antigravity Skill:" in result
    ...
    def test_no_frontmatter_returns_unchanged(self) -> None:
        ...
        result = _add_antigravity_header(content, "test")
        assert result == content


class TestAntigravityDryRun:
    """Tests for Antigravity dry-run via install_agent command."""
    ...
    def test_antigravity_dry_run(self, tmp_path: Path, plugin_dir: Path) -> None:
        """Test --agent antigravity --dry-run via CLI runner."""
        ...
        assert output["agent"] == "antigravity"
        assert output["file_count"] > 0

    def test_antigravity_install(self, tmp_path: Path, plugin_dir: Path) -> None:
        """Test --agent antigravity installs to .agents/skills/brainpalace."""
        from click.testing import CliRunner
        ...
        assert (target / "brainpalace-search" / "SKILL.md").exists()
        # AGENTS.md should land at the project root, mirroring Codex.
