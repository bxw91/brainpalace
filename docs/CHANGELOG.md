...
### Added
- **Dashboard "Ingest" tab — browse programmatically-ingested text.** A new
  instance-scoped, read-only tab lists the previously CLI-only `brainpalace
  ingest` sources (filterable, 100/page) and drills into each source's chunks.
  Two new dashboard proxy routes flip `/ingest/sources` +
...
- **`config migrate` now clears stranded pre-rename keys**, and the config
  ...
  legacy `session_extraction.mode` field are dropped (not mapped — no stale bind
  override, no surprise paid-engine re-enable); `session_extraction.quiescence_seconds`
  is preserved.
