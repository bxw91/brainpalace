...
| --mode | choice | hybrid | Retrieval mode: 'vector' (semantic similarity), 'bm25' (keyword matching), 'hybrid' (vector+bm25), 'graph' (knowledge graph relationships, requires ENABLE_GRAPH_INDEX=true), 'multi' (fusion of vector+bm25+graph), 'compute' (set-level aggregation over typed numeric records, requires ENABLE_COMPUTE=true). Default: hybrid. |
