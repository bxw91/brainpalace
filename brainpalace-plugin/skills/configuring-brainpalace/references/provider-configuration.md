...
Characteristics: OpenAI is highest-quality (3072 dims for `-large`); Cohere is
multi-language (1024 dims); Ollama runs locally, no API cost. Other supported models:
`text-embedding-ada-002` (OpenAI, legacy), `embed-english-light-v3.0` (Cohere),
`all-minilm` (Ollama).
